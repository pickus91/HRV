...
def multiScaleEntropy(timeSeries, scales, r, m):
    ...
    Inputs Arguments:         
        - timeSeries: [array] of time series data 
        - scales    : [array] of scales for MSE calculation
...
def sampEn(x, m, r): 
    '''sampEn computes sample entropy of time series 'x'. Sample entropy is a
        modification of approximate entropy (ApEn) in that (1) it does not count
        self-matching and (2) it does not depend as much on the length of
        the time series. Sample entropy algorithm details can be found in
        Pincus (1991) paper "Approximate entropy as a measure of system
        complexity".
